...
**상태: pre-alpha.** 첫 유닛들이 `@polydeukes/core`(약속(covenant) 프로토콜, ROI 텔레메트리, config 로더, fail-open/fail-closed 정책 테이블, 정규 대화 기록(canonical transcript) 질의 이음새(seam)), `@polydeukes/covenant`(run_covenant 래퍼, heredoc을 인지하는 멀티라인 Bash 분석과 쓰기 탐지 규칙(redirect/tee/`sed -i`), 경로 라우팅 디스패처, 증인(witness) 이음새를 갖춘 self-mod 메타-약속(covenant), 탐지 규칙을 읽기-전용 allowlist를 갖춘 Bash 축 판정으로 조립한 shell-mod 메타-약속(covenant), 그리고 대화 기록 이음새로 판정하며 판정 뒤에만 상의되는 시간제 증인(TTL witness) 밸브), `@polydeukes/adapter-claude-code`(PreToolUse 페이로드 → 약속(covenant) 입력 IR 변환, 주입형 dispatch seam을 갖춘 어댑터 경로 ROI 텔레메트리 배선, 디스크를 건드리지 않고 Edit/Write/MultiEdit 적용 결과를 계산하는 가상 사후 상태(virtual post-state) 파서, 그리고 시간제 증인(TTL witness)에 실물 인간 발화를 공급하는 대화 기록(transcript) JSONL 공급자), `@polydeukes/adapter-git`(커밋 표면 어댑터, `staged diff`를 약속(covenant) 입력 IR로 바꾸고 HEAD와 인덱스의 blob으로 같은 에이전트 중립 호출별 `fileChange` 증거(삭제를 1급으로 표현하는 판별 유니온(discriminated union))를 채우는 제2 어댑터, 코어 0줄 수정의 IR 중립성 증명), 그리고 umbrella `polydeukes` 패키지(`loadConfig` 디스커버리 로더와 첫 실물 `pdks` 서브커맨드 `pdks covenant check`, `block` 수위에서 그 증인(witness) 밸브는 터미널 앞의 인간만 답할 수 있는 TTY 프롬프트인 pre-commit 판정 진입점 — 수위 자체는 git 어댑터의 네임스페이스 설정 `adapters.git.enforce: block | advise`이고, `advise`는 판정을 `advised` 이벤트로 기록한 뒤 커밋을 통과시키며, 같은 네임스페이스의 `adapters.git.protectedPaths`는 공통 목록 위에 커밋 표면만 판정하는 가산 관측 범위(세션 표면은 읽지 않음)입니다)에 구현되었습니다. 나머지는 아직 청사진 단계입니다. 이 저장소는 그 초기 코어와 아키텍처 청사진, 설계 근거를 담고 있습니다. 아래는 무엇을 만들려는지에 대한 안내입니다.
