...
- **파싱 오류나 커스텀 YAML 태그**를 만나면 파일명을 담아 에러를 냅니다. 파서가 태그를
  ...
  남아야 합니다.
- **스키마 위반**이면 키와 파일명을 담아 에러를 냅니다. 코어가 소유하는 층위, 곧 최상위와
  코어가 정의한 컨테이너에서 미지 키를 거부합니다. 그래서 `protectedPaths:`를
  `protectedPath:`로 적거나 `adapters:`를 `adaptors:`로 적는 오타는 여기서 잡힙니다.
  어댑터 네임스페이스 안쪽의 어휘는 그 어댑터의 것입니다. 코어는 내용을 그대로
  통과시키고, 어댑터 자신의 검증기가 알지 못하는 것을 전체 필드 경로와 함께 거부합니다
