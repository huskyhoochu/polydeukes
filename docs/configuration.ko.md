...
> 프리알파 단계입니다. 이 문서는 지금 출시된 설정 표면(스키마 v2, 로더, 내장 술어 4종)을
...
- **`block`**은 covenant를 깨는 스테이징 변경이 있으면 커밋을 차단합니다(exit 2). 통과하는
  길은 유예 밸브뿐입니다. 사람이 TTY 프롬프트에 토큰 전체를 입력해야 합니다.
...
통과·차단·우회·권고·스킵 모든 판정이 한 줄씩 기록을 남깁니다. 텔레메트리는 의도적으로
...
조립된 어댑터 어느 쪽도 알아보지 못하는 증거 키는 조립 시점에 닫히는 쪽으로 실패하므로,
오타가 어댑터 어휘로 위장하는 일은 없습니다.
...
**가장 싼 통과 경로가 곧 정직한 경로입니다.** 유예와 달리 이 증거는 AI 자신의 표면에
...
우회는 없습니다. 커밋 표면이 `adapters.git.enforce: advise`라면 판정은 `advised`로
