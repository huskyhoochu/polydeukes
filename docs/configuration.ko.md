...
**맥락족 규율은 커밋 표면에서 빠집니다.** 커밋에는 들여다볼 세션이 없으므로,
`pdks covenant check`가 판정기를 조립할 때 `requirePrecedent` 항목을 제외합니다. 증거
채널이 없는 채로 판정하면 매치하는 커밋이 전부 막히고 정당한 통과 경로가 사라지기
때문입니다. 이 제외는 조용하지 않습니다. 제외한 항목마다 `skipped` 텔레메트리 기록이
자기 `id`를 label로 달고 한 건씩 남으므로, 이 표면에서 아무 일도 하지 않은 규율은
데이터에 그렇게 드러납니다.
