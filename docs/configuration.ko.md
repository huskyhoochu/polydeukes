...
`disciplines:` 항목의 기본 수위는 **advise**입니다. 위반은 해당 규율의 `id`와 함께
...
항목(도구 축과 셸 축의 변경, 읽기 전용 머리가 없는 언급), 세션 전사, 그리고 조립
자체입니다. 설정이 없거나 모호하거나 무효인 경우, 빌드되지 않은 판정기, 파싱할 수 없는
페이로드, 답하지 못한 라우팅이 여기 속합니다. 이것들은 어느 수위에서든 닫히는 쪽으로
실패합니다. 아무거나 통과시키는 죽은 게이트야말로 가장 값싼 우회이기 때문입니다. 커밋
표면의 `adapters.git.enforce: advise`는 보호 경로 판정까지 `advised`로 완화합니다. 관측자의
설정입니다. 판정할 수 없는 조립은 그래도 닫히는 쪽으로 실패합니다.
