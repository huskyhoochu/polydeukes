...
직접 제공합니다. 네임스페이스 *안의* 미지 키는 그 어댑터의 검증기가 전체 필드 경로를
에러에 담아 거부합니다.
...
- **`block`** — covenant를 깨는 스테이징 변경은 커밋을 차단합니다(exit 2). 통과하는
...
- **`advise`** — 커밋 표면이 차단 없는 백스톱이 됩니다. 스테이징 변경에 내려진 판정은
...
통과·차단·우회·권고 모든 판정이 한 줄씩 기록을 남깁니다. 텔레메트리는 의도적으로
...
선택적으로 `why`(이유를 규칙 옆에 보관)를 가집니다. `forbid` 항목에 한해 `in`(판정할
파일 glob)과 `except`(그 범위에서 덜어낼 glob)도 쓸 수 있습니다.
