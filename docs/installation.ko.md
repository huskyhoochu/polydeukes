...
> 발행된 패키지의 실제 측정 동작입니다.

이 문서는 시작하기 층입니다. 빈 프로젝트에서 첫 호출이 판정받는 순간까지를 다룹니다.

devDependency 하나, 표면마다 명령 하나면 됩니다. 설치하는 것은 우산(umbrella) 패키지
`polydeukes` 하나뿐입니다. 코어와 판정기와 어댑터를 자기 의존성으로 안고 오며, CLI는
`pdks`(`polydeukes`의 별칭)입니다.

**표면은 둘이고 쓰이는 상황이 다릅니다. 프로젝트를 개발하는 방식에 맞는 쪽을
고르세요.** Claude Code에서 AI 파트너와 함께 만드는 프로젝트는 **세션 표면**을
...
같은 설정 어휘를 강제하지만 답하는 상황이 다르므로, 한 프로젝트에 둘을 함께 배선할
일반적인 이유는 없습니다.
...
생성물은 다섯이고, 이미 있는 것은 절대 덮어쓰지 않습니다. 있는 것은 보고하고 지킵니다.
훅과 설정과 규율 파일은 그대로 두고, settings 파일은 병합하며, `.gitignore`에는 덧붙이기만
...
않으므로, 편집을 기다리는 동안 잘못된 판정 결과를 낼 수는 없습니다. 다만 `languages`는
...
- **밸브는 TTY 프롬프트입니다.** 강제 수준이 기본값 `block`이면 보호 대상 변경을
  스테이징한 커밋이 프롬프트에서 멈추고, 터미널 앞의 사람만 답할 수 있습니다. 훅 러너가
  그 프롬프트를 삼키지 않게 설정하세요(lefthook은 `interactive: true`가 필요합니다).
- **이 표면에서 판정하는 규율 계열은 둘입니다.** staged diff는 파일 변경만 싣고 다른
  것은 싣지 않으므로, 보호 목록과 델타 계열·경로 계열(`forbid`·`immutable`)은 온전히
  판정합니다. 명령 계열(`forbidCommand`) 항목은 staged diff에 읽을 명령줄이 없어 이
  표면에 조립되지 않고, 맥락 계열(`requirePrecedent`) 항목은 `skipped`로 기록됩니다. 이
  둘은 AI 파트너의 세션이 판정될 수 있는 곳에 선언하세요.
- **커밋 표면에는 자기만의 추가 보호 목록이 있습니다.** 자유롭게 편집해도 되지만 이력으로
...
두 표면 모두 같은 밸브를 자기 상황의 철자로 싣습니다. 밸브는 판정 결과 **뒤**에 있어
실제로 차단된 판정만 증언으로 열 수 있고, 모든 허용은 `witnessed`로 기록되며 조용히 지나가는
법이 없습니다.
