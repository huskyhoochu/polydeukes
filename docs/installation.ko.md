...
생성물은 넷이고, 이미 있는 것은 절대 덮어쓰지 않습니다. 있는 것은 보고만 하고 그대로
두므로 재실행은 언제나 안전합니다.
...
| `.gitignore` | `.polydeukes/` 한 줄을 덧붙입니다. 텔레메트리는 로컬 관측 데이터라 이력에 들어가지 않습니다. |
