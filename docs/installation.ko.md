...
- **이 표면에서는 선언이 판정합니다.** staged diff는 파일 변경만 싣고 다른 것은 싣지
  않으므로, 보호 목록과 변경 위의 모든 `declare` 항목은 온전히 판정합니다. `command`에
  범위를 건 선언은 staged diff에 읽을 명령줄이 없어 이 표면에서 아무것도 관측하지 않고,
  세션을 읽는 선언(`precedent`와 다른 이력 기전)은 `skipped`로 기록됩니다. 이런 선언은 AI
  파트너의 세션이 판정될 수 있는 곳에 두세요.
