...
**판정의 설치 단위가 이 패키지입니다.** 우산의 표면 없이 판정만 원하는 임베더는 이 패키지를
설치하고 `@polydeukes/core`를 함께 제공합니다. core가 여기서 `peerDependency`라서 어휘가
복제되지 않고 공유됩니다. 보통의 소비자에게는 우산이 그 peer를 만족시킵니다.
...
**`disciplines:` 항목은 정확히 한 계열에 속하고**, 어느 술어 키를 들었는지가 그것을
정합니다. 계열은 판정에 필요한 증거를 정하고, 그것이 곧 어느 표면에서 판정될 수 있는지를
정합니다.
...
- **맥락 계열은 세션 없이 판정할 수 없습니다.** 커밋 표면에는 세션이 없으므로 매치된
  `requirePrecedent` 항목은 언제나 `skipped`를 기록합니다. 그 표면의 항구적 조건입니다.
- **명령 계열은 커밋 표면에 없고, 없다는 기록도 남지 않습니다.** 스테이징된 diff에는 판정할
  명령줄이 없으므로 `forbidCommand` 항목은 컴파일 전에 걸러집니다. 맥락 계열과 달리
...
- **판정할 수 없는 항목은 스킵 등록으로 컴파일됩니다.** 라우팅은 그대로 두고 본체를 두지
  ...
  그것을 일으킨 설정을 고칠 길이 사라지기 때문입니다. 스킵이 아닌 한 형상이 있습니다.
  술어를 아직 어느 계열도 판정하지 않는 항목 — core는 받아들였고 covenant에는 분기가
  없는 — 은 라우팅되고, 본체가 판정 불가(exit 2)로 답합니다. 통과로 답하지 않습니다.
