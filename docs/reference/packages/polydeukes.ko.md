...
- **컴파일러가 해석하지 못하는 선언은 로드되지 않습니다.** 등재 표 밖의 단계, 단계의 키 밖의
  인자, 쌍/단일 불일치가 그 경우입니다. 예약 기전이나 카탈로그가 거부하는 축·관계와 마찬가지로
  설정을 읽는 단계에서 파일을 거부하고(exit 2) 해당 항목을 알리며, skip 행은 남지 않습니다.
  세션 안에서도 설정 파일은 고칠 수 있습니다. 결과가 로드되는 Edit 또는 Write 한 번은
  통과합니다. 판정 시점에 세계에 없는 소스는 다른 경우입니다. 선언 자신의 `supply` 정책이
  처분하고, 정책이 없으면 본체가 판정 불가(exit 2)로 답합니다. 통과로 답하지 않습니다.
