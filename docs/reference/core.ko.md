...
> 텔레메트리 수집기가 여기 있습니다.
>
> 알파입니다. 우산의 전이 의존이라서 설치하지 않고, 판정 본체를 직접 쓰는 경우가 아니면
> 불러오지도 않습니다. 소비자 진입점은 [`polydeukes`](./polydeukes.ko.md)입니다.
...
| 페일 정책 | 실패 종류마다 페일오픈과 페일클로즈를 가르는 표 하나 |
...
판정 본체가 말하는 계약이고, 소비자가 이 패키지에서 정당하게 손댈 수 있는 유일한
부분입니다. 판정 본체를 직접 쓴다는 것은 표준 입력에서 `CovenantInput`을 읽고 종료 코드로
답한다는 뜻입니다.
...
- **기본 전사는 아무것도 하지 않습니다.** 실제 전사를 주입하지 않은 소비자는 "아무 일도
  ...
  실제 전사는 어댑터 뒤에 있습니다.
- **페일오픈은 텔레메트리 하나뿐입니다.** 기록이 실패해도 판정은 바뀌지 않습니다. 표에 든
