...
두 표지가 함께 있어야 한다거나 한 표지가 있어야 한다는 관계처럼 hunk 밖의 줄에 기대는 관계는
hunk만으로 판정합니다. 그래서 표지 하나를 지운 diff는 통과하고, 이미 표지가 있는 파일에 줄 하나를
더한 diff는 위반이 될 수 있습니다. 같은 선언을 세션 표면은 파일 전체로 판정합니다. `source`로
지정한 파일은 아래의 세계 축에서 전체를 읽지만, `source`는 고정 경로를 가리킬 뿐 바뀌는 파일
자체를 가리키지 못합니다. 생성과 삭제는 전체 텍스트를 담습니다.
