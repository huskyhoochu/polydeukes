...
`@polydeukes/core`는 `peerDependency`로 부릅니다. 어휘를 판정자와 공유할 뿐 여기서 한 벌
더 설치하지 않습니다.
...
| worktree | HEAD blob | 디스크의 바이트 | 미추적(무시되지 않은) 파일은 `added`로 합류하고, 디스크에서 사라진 파일은 HEAD에 있었든 인덱스에만 있었든 `deleted`입니다. 읽을 수 없는 경로(깨진 심볼릭 링크)는 내용 null로 경로만 판정합니다 |
...
**가산 범위가 가산인 데는 이유가 있습니다.** 강제 수준이 관측자의 것이듯 범위도 관측자의
...
실행은 어느 강제 수준에서도 종료 `2`로 fail-closed합니다.
...
- **맥락 계열은 여기서 판정할 수 없습니다.** `requirePrecedent`는 세션 이력이 필요한데 커밋에는
...
- **밸브에는 터미널 앞의 사람이 필요합니다.** TTY가 없으면 프롬프트도 없고 통과할 길도
  없습니다. CI 실행과 에이전트가 띄운 `git commit`이 같은 닫힌 관문에 닿습니다. 무엇도
