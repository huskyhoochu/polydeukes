// ─────────────────────────────────────────────────────────────────────────────
// VENDORED from Claude Code's built-in code-review workflow (copied 2026-07-22).
//
// Why a copy exists here: the built-in `/code-review` skill is user-invocable only
// (`disable-model-invocation`), so the agent cannot run it. That left the `/ticket`
// loop's PR-review phase stalling on a manual step every time. A workflow script a
// skill points at IS invocable, so `.claude/skills/ticket` now launches this file
// via `Workflow({name: "pdks-code-review"})` and the loop runs end to end.
//
// The name is deliberately NOT `code-review`: a distinct name keeps this fork and
// Claude Code's own workflow unambiguous — a call site naming `pdks-code-review` is
// visibly ours, and the two can never be confused for one another at resolution time.
//
// Consequences of vendoring, know them before editing:
//   - upstream improvements do NOT reach this copy; it is frozen at the copy date
//   - `/code-review` typed by a human still runs Claude Code's own version, so the
//     two can drift — when they disagree, neither is automatically right
//   - the script is repo-agnostic as copied (no paths, languages, or conventions
//     baked in; CLAUDE.md files are discovered at runtime), so it stays portable
//     unless someone edits that property away
//   - this file is EXCLUDED from Biome (biome.json files.includes): repo formatting
//     would churn every line and bury the real divergences from upstream
//
// The args contract is unchanged from upstream: "<level> [target]", level being
...
if (P.sweep) {
  ...
  const sweep = await agent(
    ...
    { label: "sweep", phase: "Sweep", schema: CANDIDATES_SCHEMA, model: "opus" }
  )
