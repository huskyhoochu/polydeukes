...
- **델타 계층(delta layer).** 파일의 편집 전후 쌍을 비교해 새 위반만 보는 순수 판정입니다.
  이미 있던 부채는 사면하고, 이번 편집이 새로 추가한 매치만 약속을 깹니다. 맥락
  계열(context family)의 `when` 트리거가 이 위에 세워집니다.
- **표준 규율(discipline) 라이브러리.** config의 `disciplines:` 항목이 코드 한 줄 없이
  강제가 됩니다. `forbidCommand`(명령 계열), `requirePrecedent`(맥락 계열, 아래 항목),
  `declare`(선언 계열 — 아래 엔진이 판정하는 대수 선언, 파일 변경마다 세계 하나를 공급)의
  세 술어입니다.
