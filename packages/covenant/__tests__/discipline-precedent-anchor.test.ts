 ...
 */
type Decision = 'found' | 'missing' | 'both' | 'none';

function precedentDecision(calls: ObservedCall[], entry: DisciplineEntry = whenEntry): Decision {
...
describe('COVENANT-13b §4.3 command evidence — anchored at a simple command (AC 7)', () => {
  it('refuses a quoted mention inside an echo, while the same words alone still qualify', () => {
    ...
    expect(precedentDecision([shellCall('echo "npm view yaml"')])).toBe('missing');
  ...
  it('refuses a mention parked behind a comment marker', () => {
    ...
    expect(precedentDecision([shellCall('ls -la # npm view yaml')])).toBe('missing');
  });

  it('accepts the command when it is the SECOND simple command of a chain', () => {
    ...
    // simple command.
    expect(precedentDecision([shellCall('cd pkg && npm view yaml')])).toBe('found');
  });

  it('refuses a chained command the shell never reached, though the anchor accepts its shape', () => {
    ...
    expect(precedentDecision([shellCall('false && npm view zzz', 'failed')])).toBe('missing');
    expect(precedentDecision([shellCall('false && npm view zzz', 'succeeded')])).toBe('found');
  });

  it('refuses a heredoc body that spells the command, reading only the words', () => {
    ...
    expect(precedentDecision([shellCall(heredoc)])).toBe('missing');
    expect(precedentDecision([shellCall('npm view yaml')])).toBe('found');
  });

  it('refuses a simple command with no words at all', () => {
    ...
    expect(precedentDecision([shellCall('> out.txt')], optionalEntry)).toBe('missing');
    expect(precedentDecision([shellCall('npm view yaml')], optionalEntry)).toBe('found');
  });

  it('refuses an assignment prefix — a documented over-block, pinned so it stays deliberate', () => {
    ...
    expect(precedentDecision([shellCall('FOO=1 npm view yaml')])).toBe('missing');
    expect(precedentDecision([shellCall('npm view yaml')])).toBe('found');
  });
});
...
describe('COVENANT-13b §4.3 command evidence — anchoring by position, not by pattern rewrite (AC 9)', () => {
  it('anchors every branch of an alternation, not just the first', () => {
    ...
    expect(precedentDecision([shellCall('echo "pnpm view yaml"')], alternationEntry)).toBe(
      'missing',
    );
    expect(precedentDecision([shellCall('pnpm view yaml')], alternationEntry)).toBe('found');
  });

  it('refuses a call whose outcome the provider could not observe', () => {
    ...
    expect(precedentDecision([shellCall('npm view yaml', 'unknown')])).toBe('missing');
    expect(precedentDecision([shellCall('npm view yaml', 'succeeded')])).toBe('found');
  });
});
...
describe('COVENANT-13b §4.6 command evidence — the recovery path stays open (AC 10)', () => {
  it('reopens the gate when the required command is re-run on its own after a compound failure', () => {
    ...
    expect(precedentDecision([failedCompound])).toBe('missing');
    expect(precedentDecision([failedCompound, shellCall('npm view yaml', 'succeeded')])).toBe(
      'found',
    );
  });

  it('accepts a command the shell short-circuited past — the under-blocking half, on purpose', () => {
    ...
    expect(precedentDecision([shellCall('true || npm view yaml', 'succeeded')])).toBe('found');
  });
});
