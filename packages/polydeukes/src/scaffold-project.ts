 ...
 *
 * The first line is the `yaml-language-server` directive pointing at the schema this package
 * ships in `dist/schema/` (DIST-05 §3-b). Its value is a consumer-root-relative FILE path
 * because `$schema` is a static string an editor reads, so no module resolver runs on it, and
 * the directive is honored only at the head of the document. Being a YAML comment, the loader
 * never sees it (§5 invariant 4).
 */
const GENERATED_CONFIG = `# yaml-language-server: $schema=node_modules/polydeukes/dist/schema/polydeukes.schema.json
# Polydeukes protection policy — generated by \`pdks init claude-code\`.
#
