...
// strictest level.
import { resolveGitAdapterSettings } from '../src/settings.ts';
...
describe('resolveGitAdapterSettings — default fill (silent-relaxation is block)', () => {
  it('fills { enforce: block } when the namespace is absent (undefined)', () => {
    expect(resolveGitAdapterSettings({ namespace: undefined })).toEqual({
      enforce: 'block',
      protectedPaths: [],
    });
  });
  ...
  it('fills { enforce: block } when enforce is absent (empty object)', () => {
    ...
    expect(resolveGitAdapterSettings({ namespace: {} })).toEqual({
      enforce: 'block',
      protectedPaths: [],
    });
  });
...
describe('resolveGitAdapterSettings — verbatim pass-through', () => {
  it('returns { enforce: block } verbatim', () => {
    expect(resolveGitAdapterSettings({ namespace: { enforce: 'block' } })).toEqual({
      enforce: 'block',
      protectedPaths: [],
    });
  });
  ...
  it('returns { enforce: advise } verbatim', () => {
    ...
    expect(resolveGitAdapterSettings({ namespace: { enforce: 'advise' } })).toEqual({
      enforce: 'advise',
      protectedPaths: [],
    });
  });
...
describe('resolveGitAdapterSettings — fail-fast rejection', () => {
  it('throws on the reserved level (measure) with a field-path-named message', () => {
    ...
    expect(() => resolveGitAdapterSettings({ namespace: { enforce: 'measure' } })).toThrow(
      /adapters\.git\.enforce/,
    );
  });

  it('throws on a non-string enforce value with a field-path-named message', () => {
    expect(() => resolveGitAdapterSettings({ namespace: { enforce: 1 } })).toThrow(
      /adapters\.git\.enforce/,
    );
  });
  ...
  it('throws on an unknown key inside the namespace with a field-path-named message', () => {
    ...
    expect(() =>
      resolveGitAdapterSettings({ namespace: { enforce: 'block', extra: true } }),
    ).toThrow(/adapters\.git/);
  });
});
...
describe('resolveGitAdapterSettings — protectedPaths acceptance', () => {
  it('accepts an explicit empty additive list', () => {
    ...
    expect(resolveGitAdapterSettings({ namespace: { protectedPaths: [] } })).toEqual({
      enforce: 'block',
      protectedPaths: [],
    });
  });
  ...
  it('passes a valid additive list through verbatim while enforce still fills block', () => {
    ...
    expect(
      resolveGitAdapterSettings({ namespace: { protectedPaths: ['packages/core/src'] } }),
    ).toEqual({
      enforce: 'block',
  ...
  it('resolves enforce and protectedPaths together when both are present', () => {
    ...
    expect(
      resolveGitAdapterSettings({
        namespace: { enforce: 'advise', protectedPaths: ['packages/core/src'] },
      }),
...
describe('resolveGitAdapterSettings — protectedPaths rejection', () => {
  it('throws on a non-array protectedPaths with the exact contract message', () => {
    ...
    expect(() =>
      resolveGitAdapterSettings({ namespace: { protectedPaths: 'packages/core/src' } }),
    ).toThrow(/adapters\.git\.protectedPaths must be an array of strings/);
  });
  ...
  it('throws on a non-string element with the same message', () => {
    ...
    expect(() => resolveGitAdapterSettings({ namespace: { protectedPaths: [42] } })).toThrow(
