...
- **The adapter's own vocabulary** — `resolveGitAdapterSettings(namespace)` validates this adapter's config namespace (`adapters.git`): `enforce: block | advise`, the commit surface's enforcement level. The core validates only the container shape (one settings object per adapter) and passes contents through verbatim, so the vocabulary, its validator, and its defaults live here — absence fills `block`, unknown keys and values fail fast with the full field path.
