...
// The generated artifacts EXECUTE: the built `pdks-grok init` installs into a throwaway
// tree wired to the real install graph by symlink, and the generated delegator is spawned
// as the session host would spawn it. init.test.ts judges the same artifacts as text; only
// a spawn proves that the delegator loads this package, that `runHook` finds and spawns
// `pdks`, and that the child judges the payload against the GENERATED config.
...
/** The protected entry the generated config names; a write under it is self-mod. */
const PROTECTED_ENTRY = '.grok/hooks';
...
beforeEach(() => {
  ...
  projectRoot = realpathSync(mkdtempSync(join(tmpdir(), 'pdks-grok-init-e2e-')));
});
...
/** One camelCase-only write payload, projectRoot-relative — the proven-mutation-target branch. */
...
describe('pdks-grok init on a real install graph', () => {
  it('exits 0, leaves the four artifacts, and reports every one as skipped on a re-run', () => {
    ...
    // runner's cwd), and the two registration artifacts follow. The re-run pins that
...
describe('the generated delegator judges real payloads', () => {
  it('passes a write only the dogfooding config would block: exit 0, exactly one runner row', () => {
    ...
    const result = spawnGeneratedHook(
      writePayload(join(projectRoot, DOGFOODING_ONLY_PROTECTED), 'pre-commit:\n  commands: {}\n'),
    );
  ...
  it('blocks a camelCase write into the generated grok registration: exit 2, reason on stderr, self-mod row', () => {
    // The gate entry executing rather than grep'd: the generated config's own entry blocks
    // a mutation of the directory that registers the judge. The exact-row pin separates a
    // verdict from a fail-closed crash on the same exit code: a crash records under the
    // runner's label, never under self-mod. The child's stderr is inherited through the
    // delegator, so the break reason reaches the host.
    expect(installIntoFixture().status).toBe(0);

    const result = spawnGeneratedHook(writePayload(join(projectRoot, JSON_REL), '{}'));
