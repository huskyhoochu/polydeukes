/**
 * File-change evidence computation — turns one `write` or `search_replace` payload
 ...
 * non-mutating tools and unparseable envelopes yield `null`, never an error.
 */

import type { FileChange } from '@polydeukes/core';
import { parsePayloadEnvelope } from './payload-envelope.ts';
import { MUTATING_TOOLS } from './session-vocabulary.ts';
 ...
 * existing one a `modify` (these tools cannot delete). Evidence is singular like its
 * IR home (`toolCall.fileChange`): one payload proves at most one change, and `null`
 * means nothing provable.
 */
export function collectFileChanges(
  ...
  if (!MUTATING.has(envelope.toolName)) return null;
