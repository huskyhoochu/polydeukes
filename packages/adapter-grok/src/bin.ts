...
if (args.length === 1 && args[0] === 'init') {
  try {
    ...
    process.stderr.write(
      `pdks-grok init failed: ${error instanceof Error ? error.message : String(error)}\n`,
    );
    process.exit(2);
  }
}

process.stderr.write('usage: pdks-grok init\n');
process.exit(2);
